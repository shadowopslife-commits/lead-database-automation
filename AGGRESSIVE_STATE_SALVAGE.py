...
# open state files for append
state_files = {
    st: open(os.path.join(BASE, f"{st}.txt"), "a", encoding="utf-8", errors="ignore")
    for st in STATES
}

unknown_out = open(OUTPUT_UNKNOWN, "w", encoding="utf-8", errors="ignore")

# aggressive regex: catches Ca, 'Ca', "CA", ,Az,, etc.
state_regex = {
    st: re.compile(rf"(?i)[\'\",\[\]\s]{st}[\'\",\]\s]|,{st},|,{st}\s| {st},")
    for st in STATES
}
...
print("=== AGGRESSIVE STATE SALVAGE STARTED ===", flush=True)
...
print("=== AGGRESSIVE SALVAGE COMPLETE ===", flush=True)
